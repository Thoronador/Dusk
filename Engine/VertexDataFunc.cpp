                                ...
                                const Ogre::Vector3 &scale)
{
  ...
  const bool useSoftwareBlendingVertices = entity->hasSkeleton();

  /*
  // _updateAnimation() is not const, so we don't call it here to preserve
  // const-ness of entity. Anyway, _updateAnimation() will only update once per
  // frame, and collision checks are usually made before the animation is
  // changed, so this should not be a problem and should not lead to invalid
  // vertex values.
